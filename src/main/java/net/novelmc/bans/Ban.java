...
public class Ban {
    ...
    public String constructBanMessage(Player player, String reason, String banID) {
        ...
        if (!(reason.length() == 0)) {
            banMessage.append(ChatColor.GRAY).append("Reason: ").append(ChatColor.WHITE).append(getReason(player))
                    .append("\n");
        }
        banMessage.append(ChatColor.GRAY).append("Until: ").append(ChatColor.WHITE).append(formatDate(player))
                .append("\n");
    ...
    public void addBan(Player player, CommandSender sender, String banID, String reason, Date duration, String type) {
        ...
        plugin.banConfig.set(player.getUniqueId().toString() + ".ip", Objects.requireNonNull(player.getAddress())
                .getHostString());
